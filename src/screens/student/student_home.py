...
def StudentHome(page: ft.Page):
    ...
    doc_name_text = ft.Text("Doc Name: Loading...", weight=ft.FontWeight.BOLD, color="black")
    status_label_text = ft.Text("สถานะ :", color="black54")
    status_value_text = ft.Text("Loading...", size=16, weight=ft.FontWeight.BOLD, color="black")
        ...
        if result["success"]:
            ...
            for act in data["activities"]:
                act_item = ft.Container(
                    content=ft.Row([
                        ft.Container(
                            content=ft.Icon(ft.Icons.FOLDER_OUTLINED, color="black54", size=20),
                            bgcolor="#F0F0F0", padding=10, border_radius=10
                        ),
                        ft.Column([
                            ft.Text(act["title"], size=14, color="black", weight=ft.FontWeight.W_500),
                            ft.Text(f"Status : {act['status']}", size=12, color="black54")
                        ], spacing=2)
                    ]),
                    bgcolor="#FAFAFA", padding=15, border_radius=10,
    ...
    status_card = ft.Container(
        ...
        bgcolor="white", padding=20, border_radius=15, width=float("inf"),
    ...
    bottom_nav = ft.Container(
        content=ft.Row([
            ft.IconButton(ft.Icons.PERSON_OUTLINE, icon_color="#EF3961", icon_size=30),
            ft.Container(
                content=ft.Icon(ft.Icons.HOME_OUTLINED, color="#EF3961", size=30),
                bgcolor="#1A1A2E", padding=15, border_radius=50
            ),
            ft.IconButton(ft.Icons.CHAT_BUBBLE_OUTLINE, icon_color="#EF3961", icon_size=30),
        ], alignment=ft.MainAxisAlignment.SPACE_AROUND),
        bgcolor="#F8F8F8", padding=ft.padding.only(top=10, bottom=30)
    )
    ...
    main_content = ft.Container(
        content=ft.Column([
            ...
            ft.Text("Lastest Activities", size=20, weight=ft.FontWeight.BOLD, color="black"),
            activities_card,
